...
const AddVarietals: React.FC<AddVarietalsProps> = ({
  ...
  const [wineVarietals, setWineVarietals] = useState<WineVarietal[] | null>(
    null,
  );
  ...
  } = trpc.wines.getWineVarietals.useQuery(
    { id: id as number },
    { enabled: id != null },
  );
  ...
  useEffect(() => {
    if (varietalWineData) {
      ...
      if (wineVarietals) {
        const varietalNames = wineVarietals.map(
          (wineVarietal) => wineVarietal.varietal.name,
        );
        const varietalIds = wineVarietals.map(
          (wineVarietal) => wineVarietal.varietal.id,
        );
  ...
  return (
    <div className="mb-4">
      ...
      <div className="relative">
        ...
        />
        {isSelectVisible && varietalIsLoading && <p>Loading...</p>}
          ...
          <ul className="absolute z-10 mt-2 w-full border border-gray-300 rounded-lg bg-white shadow-md">
            {varietals &&
              varietals
                .filter(
                  (varietal) =>
                    !selectedVarietals.includes(varietal.name.toLowerCase()),
                )
                .filter((varietal) =>
                  varietal.name
                    .toLowerCase()
                ...
                .map((varietal) => (
                  <li
                    ...
                    onClick={() =>
                      handleVarietalSelect({
                        name: varietal.name.toLowerCase(),
                        id: varietal.id,
                      })
                    }
                  >
